...
{
	public class AcadSplineDictionary : NodeObject
	{
		private const string cstrClassName = "AcadSplineDictionary";
		...
		internal int FriendLetDatabaseIndex
		{
			set
			{
				...
				AcadSpline dobjAcadSpline = default(AcadSpline);
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					AcadObject robjAcadObject = dobjAcadSpline;
					string nrstrErrMsg = "";
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadSpline = (AcadSpline)robjAcadObject;
					dobjAcadSpline.FriendLetDatabaseIndex = value;
				}
				dobjAcadSpline = null;
			}
		}
		...
		internal int FriendLetDocumentIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadSpline = (AcadSpline)robjAcadObject;
					dobjAcadSpline.FriendLetDocumentIndex = value;
				}
				dobjAcadSpline = null;
			}
		}
		...
		internal int FriendLetApplicationIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadSpline = (AcadSpline)robjAcadObject;
					dobjAcadSpline.FriendLetApplicationIndex = value;
				}
				dobjAcadSpline = null;
			}
		}
		...
		internal double FriendLetOwnerID
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadSpline = (AcadSpline)robjAcadObject;
					dobjAcadSpline.FriendLetOwnerID = value;
				}
				dobjAcadSpline = null;
			}
		}
		...
		public AcadSplineDictionary()
		{
			mblnOpened = true;
			base.FriendLetNodeImageEnabledID = 289;
			base.FriendLetNodeImageDisabledID = 290;
			base.FriendLetNodeName = "Splinekurven";
			base.FriendLetNodeText = "B Splinekurven";
		...
		{
			AcadSpline dobjAcadSpline2 = default(AcadSpline);
		...
		internal void FriendAdd(ref AcadSpline robjAcadSpline)
		{
			robjAcadSpline.FriendLetNodeParentID = base.NodeID;
			checked
			{
				mlngEntityIndex++;
				AcadSpline acadSpline = robjAcadSpline;
				mobjDictEntities.Add("K" + Conversions.ToString(acadSpline.ObjectID), acadSpline.ObjectID);
				acadSpline = null;
			}
		}
		...
		internal object FriendGetMinMaxCoords()
		{
			...
			AcadSpline dobjObject = default(AcadSpline);
			for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
			{
				double ddblCurObjectID = Conversions.ToDouble(NewLateBinding.LateIndexGet(dvarItems, new object[1]
				{
				...
				string nrstrErrMsg = "";
				database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
				dobjObject = (AcadSpline)robjAcadObject;
		...
		public AcadSpline Item(object vvarValue)
		{
			if (mobjDictEntities.Count > 0)
			{
				...
				{
					AcadSpline dobjAcadSpline4 = default(AcadSpline);
					switch (Information.VarType(RuntimeHelpers.GetObjectValue(vvarValue)))
					{
						case VariantType.Double:
							{
								...
								if (mobjDictEntities.ContainsKey("K" + Conversions.ToString(ddblObjectID2)))
								{
									...
									AcadObject robjAcadObject = dobjAcadSpline4;
									string nrstrErrMsg = "";
									bool flag = dobjAcadDatabase.FriendObjectIdToObject(vdblObjectID3, ref robjAcadObject, ref nrstrErrMsg);
									dobjAcadSpline4 = (AcadSpline)robjAcadObject;
									if (flag)
									{
										return dobjAcadSpline4;
									}
								}
								break;
							}
			...
			AcadSpline Item = default(AcadSpline);
			return Item;
		}
	}
