...
{
	public class AcadCircleDictionary : NodeObject
	{
		private const string cstrClassName = "AcadCircleDictionary";
		...
		internal int FriendLetDatabaseIndex
		{
			set
			{
				...
				AcadCircle dobjAcadCircle = default(AcadCircle);
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					AcadObject robjAcadObject = dobjAcadCircle;
					string nrstrErrMsg = "";
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadCircle = (AcadCircle)robjAcadObject;
					dobjAcadCircle.FriendLetDatabaseIndex = value;
				}
				dobjAcadCircle = null;
			}
		}
		...
		internal int FriendLetDocumentIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadCircle = (AcadCircle)robjAcadObject;
					dobjAcadCircle.FriendLetDocumentIndex = value;
				}
				dobjAcadCircle = null;
			}
		}
		...
		internal int FriendLetApplicationIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadCircle = (AcadCircle)robjAcadObject;
					dobjAcadCircle.FriendLetApplicationIndex = value;
				}
				dobjAcadCircle = null;
			}
		}
		...
		internal double FriendLetOwnerID
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadCircle = (AcadCircle)robjAcadObject;
					dobjAcadCircle.FriendLetOwnerID = value;
				}
				dobjAcadCircle = null;
			}
		}
		...
		public AcadCircleDictionary()
		{
			mblnOpened = true;
			base.FriendLetNodeImageEnabledID = 167;
			base.FriendLetNodeImageDisabledID = 168;
			base.FriendLetNodeName = "Kreise";
			base.FriendLetNodeText = "B Kreise";
		...
		{
			AcadCircle dobjAcadCircle2 = default(AcadCircle);
		...
		internal void FriendAdd(ref AcadCircle robjAcadCircle)
		{
			robjAcadCircle.FriendLetNodeParentID = base.NodeID;
			checked
			{
				mlngEntityIndex++;
				AcadCircle acadCircle = robjAcadCircle;
				mobjDictEntities.Add("K" + Conversions.ToString(acadCircle.ObjectID), acadCircle.ObjectID);
				acadCircle = null;
			}
		}
		...
		internal object FriendGetMinMaxCoords()
		{
			...
			AcadCircle dobjObject = default(AcadCircle);
			for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
			{
				double ddblCurObjectID = Conversions.ToDouble(NewLateBinding.LateIndexGet(dvarItems, new object[1]
				{
				...
				string nrstrErrMsg = "";
				database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
				dobjObject = (AcadCircle)robjAcadObject;
		...
		public AcadCircle Item(object vvarValue)
		{
			if (mobjDictEntities.Count > 0)
			{
				...
				{
					AcadCircle dobjAcadCircle4 = default(AcadCircle);
					switch (Information.VarType(RuntimeHelpers.GetObjectValue(vvarValue)))
					{
						case VariantType.Double:
							{
								...
								if (mobjDictEntities.ContainsKey("K" + Conversions.ToString(ddblObjectID2)))
								{
									...
									AcadObject robjAcadObject = dobjAcadCircle4;
									string nrstrErrMsg = "";
									bool flag = dobjAcadDatabase.FriendObjectIdToObject(vdblObjectID3, ref robjAcadObject, ref nrstrErrMsg);
									dobjAcadCircle4 = (AcadCircle)robjAcadObject;
									if (flag)
									{
										return dobjAcadCircle4;
									}
								}
								break;
							}
			...
			AcadCircle Item = default(AcadCircle);
			return Item;
		}
	}
}
