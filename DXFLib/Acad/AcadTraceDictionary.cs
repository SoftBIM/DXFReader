...
{
	public class AcadTraceDictionary : NodeObject
	{
		private const string cstrClassName = "AcadTraceDictionary";
		...
		internal int FriendLetDatabaseIndex
		{
			set
			{
				...
				AcadTrace dobjAcadTrace = default(AcadTrace);
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					AcadObject robjAcadObject = dobjAcadTrace;
					string nrstrErrMsg = "";
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadTrace = (AcadTrace)robjAcadObject;
					dobjAcadTrace.FriendLetDatabaseIndex = value;
				}
				dobjAcadTrace = null;
			}
		}
		...
		internal int FriendLetDocumentIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadTrace = (AcadTrace)robjAcadObject;
					dobjAcadTrace.FriendLetDocumentIndex = value;
				}
				dobjAcadTrace = null;
			}
		}
		...
		internal int FriendLetApplicationIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadTrace = (AcadTrace)robjAcadObject;
					dobjAcadTrace.FriendLetApplicationIndex = value;
				}
				dobjAcadTrace = null;
			}
		}
		...
		internal double FriendLetOwnerID
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadTrace = (AcadTrace)robjAcadObject;
					dobjAcadTrace.FriendLetOwnerID = value;
				}
				dobjAcadTrace = null;
			}
		}
		...
		public AcadTraceDictionary()
		{
			mblnOpened = true;
			base.FriendLetNodeImageEnabledID = 343;
			base.FriendLetNodeImageDisabledID = 344;
			base.FriendLetNodeName = "Bänder";
			base.FriendLetNodeText = "B Bänder";
		...
		{
			AcadTrace dobjAcadTrace2 = default(AcadTrace);
		...
		internal void FriendAdd(ref AcadTrace robjAcadTrace)
		{
			robjAcadTrace.FriendLetNodeParentID = base.NodeID;
			checked
			{
				mlngEntityIndex++;
				AcadTrace acadTrace = robjAcadTrace;
				mobjDictEntities.Add("K" + Conversions.ToString(acadTrace.ObjectID), acadTrace.ObjectID);
				acadTrace = null;
			}
		}
		...
		internal object FriendGetMinMaxCoords()
		{
			...
			AcadTrace dobjObject = default(AcadTrace);
			for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
			{
				double ddblCurObjectID = Conversions.ToDouble(NewLateBinding.LateIndexGet(dvarItems, new object[1]
				{
				...
				string nrstrErrMsg = "";
				database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
				dobjObject = (AcadTrace)robjAcadObject;
		...
		public AcadTrace Item(object vvarValue)
		{
			if (mobjDictEntities.Count > 0)
			{
				...
				{
					AcadTrace dobjAcadTrace4 = default(AcadTrace);
					switch (Information.VarType(RuntimeHelpers.GetObjectValue(vvarValue)))
					{
						case VariantType.Double:
							{
								...
								if (mobjDictEntities.ContainsKey("K" + Conversions.ToString(ddblObjectID2)))
								{
									...
									AcadObject robjAcadObject = dobjAcadTrace4;
									string nrstrErrMsg = "";
									bool flag = dobjAcadDatabase.FriendObjectIdToObject(vdblObjectID3, ref robjAcadObject, ref nrstrErrMsg);
									dobjAcadTrace4 = (AcadTrace)robjAcadObject;
									if (flag)
									{
										return dobjAcadTrace4;
									}
								}
								break;
							}
			...
			AcadTrace Item = default(AcadTrace);
			return Item;
		}
	}
	{
