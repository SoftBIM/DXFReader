...
{
	public class AcadBlockRefDictionary : NodeObject
	{
		private const string cstrClassName = "AcadBlockRefDictionary";
		...
		internal int FriendLetDatabaseIndex
		{
			set
			{
				...
				AcadBlockReference dobjAcadBlockReference = default(AcadBlockReference);
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					AcadObject robjAcadObject = dobjAcadBlockReference;
					string nrstrErrMsg = "";
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadBlockReference = (AcadBlockReference)robjAcadObject;
					dobjAcadBlockReference.FriendLetDatabaseIndex = value;
				}
				dobjAcadBlockReference = null;
			}
		}
		...
		internal int FriendLetDocumentIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadBlockReference = (AcadBlockReference)robjAcadObject;
					dobjAcadBlockReference.FriendLetDocumentIndex = value;
				}
				dobjAcadBlockReference = null;
			}
		}
		...
		internal int FriendLetApplicationIndex
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadBlockReference = (AcadBlockReference)robjAcadObject;
					dobjAcadBlockReference.FriendLetApplicationIndex = value;
				}
				dobjAcadBlockReference = null;
			}
		}
		...
		internal double FriendLetOwnerID
		{
			set
			{
				...
				for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
				{
					...
					database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
					dobjAcadBlockReference = (AcadBlockReference)robjAcadObject;
					dobjAcadBlockReference.FriendLetOwnerID = value;
				}
				dobjAcadBlockReference = null;
			}
		}
		...
		public AcadBlockRefDictionary()
		{
			mblnOpened = true;
			base.FriendLetNodeImageEnabledID = 237;
			base.FriendLetNodeImageDisabledID = 238;
			base.FriendLetNodeName = "Blockeinfügungen";
			base.FriendLetNodeText = "B Blockeinfügungen";
				...
				if (mobjDictEntities.Count > 0)
				{
					...
					if (dobjAcadDatabase2 != null)
					{
						...
						for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
						{
							...
							AcadObject robjAcadObject = dobjAcadBlockReference2;
							string nrstrErrMsg = "";
							bool flag = acadDatabase.FriendObjectIdToObject(vdblObjectID, ref robjAcadObject, ref nrstrErrMsg);
							dobjAcadBlockReference2 = (AcadBlockReference)robjAcadObject;
							if (flag)
							{
		...
		internal void FriendAdd(ref AcadBlockReference robjAcadBlockReference)
		{
			robjAcadBlockReference.FriendLetNodeParentID = base.NodeID;
			checked
			{
				mlngEntityIndex++;
				AcadBlockReference acadBlockReference = robjAcadBlockReference;
				mobjDictEntities.Add("K" + Conversions.ToString(acadBlockReference.ObjectID), acadBlockReference.ObjectID);
		...
		internal object FriendGetMinMaxCoords()
		{
			...
			AcadBlockReference dobjObject = default(AcadBlockReference);
			for (int dlngIdx = num; dlngIdx <= num2; dlngIdx = checked(dlngIdx + 1))
			{
				double ddblCurObjectID = Conversions.ToDouble(NewLateBinding.LateIndexGet(dvarItems, new object[1]
				{
				...
				string nrstrErrMsg = "";
				database.FriendObjectIdToObject(ddblCurObjectID, ref robjAcadObject, ref nrstrErrMsg);
				dobjObject = (AcadBlockReference)robjAcadObject;
		...
		public AcadBlockReference Item(object vvarValue)
		{
			if (mobjDictEntities.Count > 0)
			{
				...
				{
					AcadBlockReference dobjAcadBlockReference4 = default(AcadBlockReference);
					switch (Information.VarType(RuntimeHelpers.GetObjectValue(vvarValue)))
					{
						case VariantType.Double:
							{
								...
								if (mobjDictEntities.ContainsKey("K" + Conversions.ToString(ddblObjectID2)))
								{
									...
									AcadObject robjAcadObject = dobjAcadBlockReference4;
									string nrstrErrMsg = "";
									bool flag = dobjAcadDatabase.FriendObjectIdToObject(vdblObjectID3, ref robjAcadObject, ref nrstrErrMsg);
									dobjAcadBlockReference4 = (AcadBlockReference)robjAcadObject;
									if (flag)
									{
										return dobjAcadBlockReference4;
									}
								}
								break;
							}
			...
			AcadBlockReference Item = default(AcadBlockReference);
			return Item;
		}
	}
}
